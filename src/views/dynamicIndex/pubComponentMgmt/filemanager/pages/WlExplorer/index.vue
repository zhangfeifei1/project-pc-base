  ...
  <div class="wl-explorer" @contextmenu.prevent>
    ...
    >
      <el-form-item>
        ...
        <el-button type="primary" @click="wlDownload">下载文件</el-button>
        <submit-btn
          type="danger"
          :size="size"
          :status="load.del"
          @btn="handleDel"
        >删除</submit-btn>
        <el-button type="primary" @click="editApp">应用修改</el-button>
        <!-- solt自定义头部按钮区 -->
...
export default {
  ...
  props: {
    ...
    uploadUrl: {
      type: String,
      default: '/pmobile/dynamic/virtual/addfile'
    },
    //  是否校验上传文件
    uploadReg: {
      type: Boolean,
  ...
  methods: {
    ...
    uploadPathChange([val]) {
      ...
      // this.uploadHeaders.appid = '1552274783265'
      // this.uploadHeaders['aaa'] = 111
      // this.uploadHeaders = { 'Authorization': 'Bearer ' + token,
      //   'appid': '1552274783265' }
      // this.uploadOptions = {
      //   aimPath: pathId,
      //   parents: `${val['Parents']},${val['IdentityId']},`
      //   // parentPathId: val[this.selfProps.pathPid],
      //   // pathId
      //   // isCurrentFolder: pathId === this.file.id
      // }
    ...
    fileTypeIcon(row) {
      ...
      } else if (['json'].includes(_suffix)) {
        _path = require('./images/file_json@3x.png')
      } else if (
