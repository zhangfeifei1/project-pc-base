...
import './filemanager/plugins/element.js'
// 引入树形下拉框
import wl from './filemanager/lib/wl-vue-select'
import './filemanager/lib/wl-vue-select/lib/wl-vue-select.css'
Vue.use(wl)
// 引入文件管理器
// import WlExplorer from "wl-explorer";
import WlExplorer from './filemanager/pages/WlExplorer' // 导入文件管理器
import './filemanager/lib/wl-explorer.css'
Vue.use(WlExplorer)

import fadeIn from './filemanager/components/fade-in' // 导入文件管理器
import submitBtn from './filemanager/components/submit-btn' // 导入防抖提交组件
import { closeOtherLayout, arrayToTree } from './filemanager/util' // 导入关闭其他弹出类视图函数
import {
  getFileListApi, // 1获取文件夹列表
  getAllFoldersApi // 4获取全部文件夹
} from './filemanager/api' // 导入接口

import {
  delFileApi // 6删除文件|文件夹
} from '@/api/pubComponentApi' // 导入接口
...
export default {
  ...
  data() {
    ...
    // const vm = this;
    return {
      downloadArr: [], // 接收下载文件的参数数组
      paramsArr: [], // 接收删除文件的参数数组
      load: {
        folder: false
      }, // loading管理
      fade: {
      ...
      file_table_columns: [
        ...
        },
        {
          ...
          align: 'center',
          formatter(row) {
            if (row.Size === null || row.Type === 1) return '-'
            if (row.Size < 1024) {
              // 1024以下显示kb
              return row.Size + 'b'
            }
  ...
  methods: {
    ...
    fileUpload(file, cb) {
      ...
      this.uploadOptions['aimPath'] = file['pathId']
      this.uploadOptions['parents'] = 'pathId'

      cb()
    },
    download(data, func) {
      for (let i = 0; i < data.length; i++) {
        const params = data[i].Id
        this.downloadArr.push(params)
        console.log(this.downloadArr.toString(), '下载数据11111111111')
      }
      var downloadParam = { fileNames: this.downloadArr.toString() }
      var downloadUrl = '/pmobile/dynamic/virtual/downloadFiles'
      ...
      $config.apiUrl.headers['Accept'] = 'application/json,*/*'
      $config.apiUrl.headers['appid'] = 1552274783265
      axios({
        method: 'post',
        headers: $config.apiUrl.headers,
        url: downloadUrl,
        data: qs.stringify(downloadParam),
        responseType: 'blob'
      }).then((response) => {
    ...
    getFileList() {
      ...
      getFileListApi({ path: this.path.id }).then(({ data }) => {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          setTimeout(() => {
            ...
            const res_data = this.folder_form // 由表单数据模拟服务器返回数据，此处应有服务器返回对应实体
    ...
    fileDel(data) {
      ...
      for (let i = 0; i < normal_data_folder.length; i++) {
        this.paramsArr.push(normal_data_folder[i])
      }
      for (let j = 0; j < normal_data_file.length; j++) {
        this.paramsArr.push(normal_data_file[j])
      }
      delFileApi({ 'files': this.paramsArr.toString() }).then(res => {
