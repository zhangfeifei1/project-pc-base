      ...
      <el-card class="form">
        <json-editor ref="JsonEditor" v-model="model" :schema="schema">
          <div style="text-align:center">
            ...
            <el-button type="primary" size="small" @click="submitEditData">提交</el-button>
          </div>
        </json-editor>
      </el-card>
    </el-col>
...
import { editPublicCompJson } from '@/api/pubComponentApi'
function syncRequestData(url, param) {
  ...
  // xmlhttp.send('request=' + JSON.stringify(param))
  // debugger
...
export default {
  ...
  created() {
    ...
    this.schema = syncRequestData('/pmobile/dynamic/comp/getPublicCompJson', {
      compName: this.$store.state.dynamic.dynamicPageData.compName,
      fileName: 'data.schema'
    })
    // 获取data内容的json
    this.model = syncRequestData('/pmobile/dynamic/comp/getPublicCompJson', {
      compName: this.$store.state.dynamic.dynamicPageData.compName,
      fileName: 'data'
    })
  },
  ...
  methods: {
    ...
    submitEditData() {
      ...
      editPublicCompJson(params).then(res => {
        if (res.returnCode === '000000') {
          this.$emit('update:configDialogShow', false)
