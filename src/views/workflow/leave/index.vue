<template>
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="open2" width="500px" append-to-body>
      <leaveHistoryForm :business-key="businessKey" :leave-data="leaveData" :history-from-data="historyFromData" />
...
import { getDataByBusinessKey, selectWorkflowLeaveById } from '@/api/activiti/task'
export default {
  name: 'Leave',
  ...
  data() {
    return {
      ...
      open2: false,
      // 请假类型字典
      typeOptions: [{ dictLabel: '年假', dictValue: '0' }, { dictLabel: '病假', dictValue: '1' }, { dictLabel: '事假', dictValue: '2' }],
      // 状态字典
      stateOptions: [{ dictLabel: '进行中', dictValue: '0' }, { dictLabel: '成功', dictValue: '1' }, { dictLabel: '失败', dictValue: '2' }],
      // 查询参数
      queryParams: {
        pageNum: 1,
        pageSize: 10,
  ...
  methods: {
    ...
    getList() {
      ...
      listLeave(this.queryParams).then(response => {
        debugger
        this.leaveList = response.data.rows
    ...
    checkTheSchedule(row) {
      getDefinitionsByInstanceId({ instanceId: row.instanceId }).then(response => {
        debugger
