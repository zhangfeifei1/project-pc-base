<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="Modeler" :cell-style="cellstyle" :header-cell-style="rowClass">
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + '/activiti/processDefinition/uploadStreamAndDeployment'
      }
    }
  },
  created() {
  ...
  methods: {
    ...
    getList() {
      ...
      listDefinition(this.queryParams).then(response => {
        this.Modeler = response.data.records
    ...
    handleDelete(row) {
      this.$confirm('是否确认删除编号为"' + row.key + '"的数据项?', '警告', {
        ...
        return delDefinition({ deploymentId: row.deploymentId })
      }).then(() => {
        this.getList()
