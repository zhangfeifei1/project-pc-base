  ...
  <div class="app-container">
    <el-form v-show="showSearch" ref="queryForm" :model="queryParams" :inline="true" label-width="68px">
      <el-form-item label="流程key" prop="processDefinitionKey">
        <el-input v-model="queryParams.processDefinitionKey" placeholder="请输入流程key" size="small" />
      </el-form-item>
      <el-form-item label="发起人" prop="startUserId">
        <el-input v-model="queryParams.startUserId" placeholder="请输入发起人" size="small" />
      </el-form-item>
    ...
    <el-dialog :visible.sync="modelVisible" title="进度查询" width="1680px" append-to-body>
      <div style="position:relative;height: 100%;">
        <iframe id="iframe" :src="modelerUrl" frameborder="0" width="100%" height="720px" scrolling="auto" />
      </div>
    </el-dialog>

    <!-- 查看详细信息话框 -->
    <el-dialog :title="title" :visible.sync="open2" width="500px" append-to-body>
      <formDemo :business-key="businessKey" :history-from-data="historyFromData" />
...
import formDemo from '@/views/workflow/leave/formDemo'

import {
  getDataByBusinessKey,
  selectWorkflowLeaveById
} from '@/api/activiti/task'

import {
  selectHistoryDataPage
} from '@/api/activiti/flowWork'
export default {
  name: 'Leave',
  components: {
    formDemo
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      selectHistoryDataPage(this.queryParams).then(response => {
        // debugger
        this.leaveList = response.data.historyDataResultList
    ...
    historyFory(row) {
      ...
      getDataByBusinessKey(params).then(response => {
        this.historyFromData = response.data.historyInfo
      })
      //   var params1 = {
      //     id: row.id,
      //     businessKey: row.id
      //   }
      //   selectWorkflowLeaveById(params1).then(response => {
    ...
    checkTheSchedule(row) {
      getDefinitionsByInstanceId({
        ...
        this.modelerUrl = '/bpmnjs/index.html?type=lookBpmn&instanceId=' + row.instanceId +
            '&deploymentFileUUID=' + data.deploymentId + '&deploymentName=' + encodeURI(data.resourceName)
